...
export default function TestStart() {
  ...
  const [shownScheduledMessages, setShownScheduledMessages] = useState(
    new Set()
  );
  ...
  // Check scheduled messages based on TEST START TIME (not student start time)
  useEffect(() => {
    if (!test || isSubmitted || scheduledMessages.length === 0) return;

    const checkScheduledMessages = () => {
      // Calculate elapsed time from TEST START TIME
      const testStartTime = new Date(test.start_time);
      const now = new Date();
      const elapsedMinutes = Math.floor((now - testStartTime) / 60000);

      console.log(
        `[SCHEDULED] Checking... Test elapsed: ${elapsedMinutes} min`
      );

      scheduledMessages.forEach((msg) => {
        // Only show if:
        // 1. Time has passed (elapsedMinutes >= time_minutes)
        // 2. Message not already shown
        // 3. Current time is EXACTLY at or just passed the scheduled time (within 30 seconds)
        const isAtScheduledTime =
          elapsedMinutes >= msg.time_minutes &&
          elapsedMinutes <= msg.time_minutes + 0.5; // 30 second window
        ...
        if (
          ...
          console.log(
            `[SCHEDULED] ✅ Showed message at ${elapsedMinutes} min: "${msg.message}"`
          );
        }
      });
    };
  ...
  }, [scheduledMessages, test, isSubmitted]);

  // Poll for live messages only
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok && data.success && data.messages.length > 0) {
          data.messages.forEach((msg) => {
            ...
            // Show live message notification
            ...
            console.log(`[LIVE] ✅ Received: "${msg.message}"`);
          });
        }
      } catch (err) {
  ...
  const enterFullscreenAuto = async () => {
    try {
      ...
      // Show manual prompt if auto fails
  ...
  useEffect(() => {
    ...
    const handleFullscreenChange = () => {
      ...
      if (
        ...
        // Student exited fullscreen - show warning with countdown
  ...
  const handleRunStart = () => {
    // 1. Clear console FIRST
    clearConsole();

    // 2. Set running state
    setIsRunning(true);

    // 3. Switch to output tab on mobile
    if (typeof window !== "undefined" && window.innerWidth <= 1024) {
      setMobileTab("output");
    }

    // 4. Save code AFTER (non-blocking)
    const currentCode = codeEditorRef.current?.getValue() || "";
    if (currentCode && currentCode !== lastSavedCodeRef.current) {
      saveCodeToServer(currentCode, true); // No await - let it happen in background
    }
  };
  ...
  // In page.js - CORRECTED confirmEndTest function
  ...
  const confirmEndTest = async () => {
    ...
    try {
      // Determine output and execution status
      let outputToSave = "";
      let executedStatus = 1; // 1 = NOT executed/failed, 0 = success

      if (consoleOutput.length === 0) {
        // No output - code was not executed
        outputToSave =
          "No output - The test was submitted without executing the code.";
        executedStatus = 1; // NOT executed
      } else {
        // Has output - join all console messages
        ...
        executedStatus = hasErrors ? 1 : 0; // 0 = success, 1 = failed
      }
      ...
      if (response.ok && data.success) {
        ...
        // Route based on test settings
        if (test.auto_submit) {
          // Auto-submit: go to completed page
          router.push(`/student/tests/${testId}/completed`);
        } else if (test.show_results) {
          // Manual submit with results: go to results page
          router.push(`/student/tests/${testId}/results`);
        } else {
          // Manual submit without results: go to student dashboard
          router.push("/student");
        }
      } else {
  ...
  const handleTimerExpire = async () => {
    ...
    // Always auto-submit when timer expires
    await confirmEndTest();
  };
  ...
  return (
    <>
      ...
      <div className="student-test-interface clamp-viewport">
        {/* Toolbar and rest of UI - same as before */}
