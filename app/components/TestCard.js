...
export default function TestCard({ test, currentTime, onStatusUpdate }) {
  ...
  useEffect(() => {
    ...
    if (test.start_time) {
      ...
      setFormattedStartTime(date.toLocaleString('en-IN', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric',
        hour: '2-digit',
  ...
  useEffect(() => {
    ...
    if (isExpired && test.studentStatus === 'in-progress' && !test.hasSubmission && !hasAutoUpdated) {
      updateExpiredTest()
    }
  }, [currentTime, test, hasAutoUpdated])

  const updateExpiredTest = async () => {
    const studentId = localStorage.getItem("student_id")
    if (!studentId) return

    try {
      const response = await fetch(`${API_BASE_URL}/test/${test.test_id}/update-expired`, {
        method: 'POST',
        headers: {
  ...
  const getStatusInfo = () => {
    ...
    if (isExpired && test.studentStatus === 'in-progress' && !test.hasSubmission) {
      return {
        status: 'left',
        text: 'LEFT',
        actionText: 'View Details',
        actionPath: `/student/tests/${test.test_id}/details`,
        disabled: false,
    ...
    if (test.studentStatus === 'in-progress' && !isExpired) {
      const remainingMinutes = Math.max(0, Math.floor((endTime - now) / (1000 * 60)))
      return {
        status: 'active',
        text: 'IN PROGRESS',
    ...
    if (test.studentStatus === 'completed' || test.hasSubmission) {
      return {
        ...
        actionPath: test.show_results ? `/student/tests/${test.test_id}/results` : `/student/tests/${test.test_id}/details`,
        disabled: false,
        description: test.score !== null ? `Score: ${test.score}%` : 'Submitted successfully',
        icon: (
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className={`student-test-card student-fade-in-up ${statusInfo.urgent ? 'urgent' : ''} ${statusInfo.status === 'left' ? 'left' : ''}`}>
      ...
      <div className="student-test-content">
        ...
        <div className="student-test-details">
          <div className="student-test-detail-row">
            ...
            <span className="student-test-detail-value" style={{ color: 'var(--student-primary-blue)' }}>
              {test.language}
            </span>
          </div>
          ...
          <div className="student-test-detail-row">
            ...
            <span className="student-test-detail-value">{test.duration} minutes</span>
          </div>
          
          {statusInfo.status === 'live' && statusInfo.remainingMinutes !== undefined && (
            <div className="student-test-detail-row">
              <span className="student-test-detail-label">
                <svg className="w-4 h-4 inline mr-1 animate-pulse" fill="currentColor" viewBox="0 0 24 24">
                  <circle cx="12" cy="12" r="8" />
                </svg>
                Time Remaining:
              </span>
              <span className="student-test-detail-value" style={{ color: 'var(--student-danger-color)', fontWeight: 'bold' }}>
          ...
          {statusInfo.status === 'left' && (
            <>
              <div className="student-test-detail-row">
                <span className="student-test-detail-label" style={{ color: '#ef4444' }}>
                  <svg className="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
                  Status:
                </span>
                <span className="student-test-detail-value" style={{ color: '#ef4444', fontWeight: 'bold' }}>
              ...
              {test.duration_taken && (
                <div className="student-test-detail-row">
                  ...
                  <span className="student-test-detail-value">{test.duration_taken} min</span>
                </div>
              )}
              {test.progress && test.progress > 0 && (
                <div className="student-test-detail-row">
                  <span className="student-test-detail-label">Progress:</span>
                  <span className="student-test-detail-value">{test.progress}%</span>
                </div>
              )}
            </>
          )}
          
          {test.studentStatus === 'in-progress' && statusInfo.status === 'active' && (
            <>
              <div className="student-test-detail-row">
                <span className="student-test-detail-label">Progress:</span>
                <span className="student-test-detail-value" style={{ color: 'var(--student-warning-color)' }}>
                  {test.progress || 0}%
                </span>
              </div>
              {test.duration_taken && (
          ...
          {(test.studentStatus === 'completed' || test.hasSubmission) && test.score !== null && test.score !== undefined && (
            <div className="student-test-detail-row">
              <span className="student-test-detail-label">Score:</span>
              <span className="student-test-detail-value" style={{ 
                color: test.score >= 80 ? 'var(--student-success-color)' : 
                       test.score >= 60 ? 'var(--student-warning-color)' : 
        ...
        {test.studentStatus === 'in-progress' && statusInfo.status === 'active' && (
          <div className="student-progress-wrapper" style={{ marginBottom: 'var(--student-space-md)' }}>
            <div className="student-progress-bar">
              <div 
                className="student-progress-fill"
                style={{ 
                  width: `${test.progress || 0}%`,
                  background: (test.progress || 0) > 75 ? 'var(--student-gradient-success)' : 
                             (test.progress || 0) > 50 ? 'var(--student-gradient-primary)' : 
                             (test.progress || 0) > 25 ? 'var(--student-gradient-warning)' : 
                             'var(--student-gradient-danger)'
                }}
              />
            </div>
            <div style={{ fontSize: '0.75rem', color: 'var(--student-text-muted)', textAlign: 'center', marginTop: '0.25rem' }}>
        ...
        {statusInfo.actionPath ? (
          <Link href={statusInfo.actionPath}>
            <button className={`student-test-action-btn ${statusInfo.status} ${statusInfo.urgent ? 'urgent' : ''}`}>
              {statusInfo.icon}
              {statusInfo.actionText}
            </button>
          </Link>
        ) : (
          <button className={`student-test-action-btn ${statusInfo.status}`} disabled={statusInfo.disabled}>
            {statusInfo.icon}
            {statusInfo.actionText}
          </button>
        )}
