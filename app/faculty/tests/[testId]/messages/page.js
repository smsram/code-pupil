...
const MESSAGE_TEMPLATES = [
  { id: 1, title: '⏰ Time Reminder', text: 'You have 15 minutes remaining. Please complete your test.' },
  { id: 2, title: '💡 Tip', text: 'Remember to save your code frequently and test it before submitting.' },
  { id: 3, title: '🎯 Focus', text: 'Stay focused and do your best! You\'ve got this!' },
  { id: 4, title: '⚠️ Warning', text: 'Please avoid copying code from external sources. Plagiarism will be detected.' },
  { id: 5, title: '✅ Encouragement', text: 'Great work! Keep going and finish strong!' },
  { id: 6, title: '📝 Submit Reminder', text: 'Don\'t forget to click Submit when you\'re done!' },
]
...
export default function TestMessages() {
  ...
  const loadMessageHistory = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/test/${testId}/message-history`)
  ...
  const sendLiveMessage = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/test/${testId}/live-message`, {
        method: 'POST',
  ...
  if (isLoading) {
    return <LoadingOverlay active={true} message="Loading..." type="spinner" blur={true} />
  }
  ...
  return (
    <>
      <LoadingOverlay active={isSendingLive} message="Sending message..." type="spinner" blur={true} />
      
      <div className="messages-fade-in" style={{ pointerEvents: "auto" }}>
        <div className="messages-dashboard-card" style={{ pointerEvents: "auto" }}>
          {/* Back Button */}
          ...
          <div className="messages-section">
              ...
              {showTemplates && (
                <div className="messages-templates-grid">
                  {MESSAGE_TEMPLATES.map(template => (
                    ...
                    >
                      <div className="messages-template-title">{template.title}</div>
                      <div className="messages-template-text">{template.text}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            <button
              ...
              disabled={isSendingLive || !liveMessageText.trim() || charCount > MAX_CHARS}
                ...
                {messageHistory.map((msg, index) => (
                  <div key={index} className="messages-history-item">
                    ...
                    <div className="messages-history-content">
                      ...
                      <div className="messages-history-time">{formatDateTime(msg.sent_at)}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="messages-info-box">
            ...
            <div>
              <strong>Note:</strong> Messages are sent instantly to all students currently taking the test. 
              Students will see a notification banner with your message.
            </div>
          </div>
        </div>
      </div>
