...
const EditTest = () => {
  ...
  const fetchTestDetails = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        // Parse start_time to date and time
  ...
  const addScheduledMessage = (e) => {
    e.preventDefault();
    e.stopPropagation();
    setScheduledMessages((prev) => [...prev, { time_minutes: "", message: "" }]);
  };
  ...
  const handleSubmit = async (status) => {
    ...
    setIsSaving(true);
    setLoadingMessage(status === 'published' ? "Publishing test..." : "Saving changes...");
  ...
  return (
    <>
      <LoadingOverlay active={isSaving} message={loadingMessage} type="spinner" blur={true} />
          ...
          <form style={{ display: "grid", gap: "2rem", pointerEvents: "auto" }}>
            ...
            >
              ...
              <div style={{ marginTop: "1rem" }}>
                <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer", userSelect: "none", pointerEvents: "auto" }}>
                  <input type="checkbox" name="fullscreenMode" checked={formData.fullscreenMode} onChange={handleInputChange} disabled={isSaving} style={{ cursor: "pointer", pointerEvents: "auto" }} />
              ...
              <div style={{ marginTop: "0.5rem" }}>
                <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer", userSelect: "none", pointerEvents: "auto" }}>
                  <input type="checkbox" name="autoSubmit" checked={formData.autoSubmit} onChange={handleInputChange} disabled={isSaving} style={{ cursor: "pointer", pointerEvents: "auto" }} />
                  <span style={{ color: "#e2e8f0" }}>Auto-submit when time expires</span>
                </label>
              </div>

              <div style={{ marginTop: "0.5rem" }}>
                <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer", userSelect: "none", pointerEvents: "auto" }}>
                  <input type="checkbox" name="showResults" checked={formData.showResults} onChange={handleInputChange} disabled={isSaving} style={{ cursor: "pointer", pointerEvents: "auto" }} />
                  <span style={{ color: "#e2e8f0" }}>Allow students to view results after completion</span>
                </label>
              </div>

              <div style={{ marginTop: "0.5rem" }}>
                <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer", userSelect: "none", pointerEvents: "auto" }}>
                  <input type="checkbox" name="waitUntilEnd" checked={formData.waitUntilEnd} onChange={handleInputChange} disabled={isSaving} style={{ cursor: "pointer", pointerEvents: "auto" }} />
                  <span style={{ color: "#e2e8f0" }}>Wait until exam ends (no submit button enabled)</span>
                </label>
              </div>

              <div style={{ marginTop: "0.5rem" }}>
                <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer", userSelect: "none", pointerEvents: "auto" }}>
                  <input type="checkbox" name="allowCopy" checked={formData.allowCopy} onChange={handleInputChange} disabled={isSaving} style={{ cursor: "pointer", pointerEvents: "auto" }} />
              ...
              <div style={{ marginTop: "0.5rem" }}>
                <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", cursor: "pointer", userSelect: "none", pointerEvents: "auto" }}>
                  <input type="checkbox" name="allowPaste" checked={formData.allowPaste} onChange={handleInputChange} disabled={isSaving} style={{ cursor: "pointer", pointerEvents: "auto" }} />
                ...
                {scheduledMessages.map((msg, index) => (
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => updateScheduledMessage(index, "time_minutes", e.target.value)}
                      disabled={isSaving}
                    />
                    <input
                      type="text"
                      ...
                      style={{ padding: "0.5rem", pointerEvents: "auto", cursor: "text" }}
                      value={msg.message}
                      onChange={(e) => updateScheduledMessage(index, "message", e.target.value)}
                      disabled={isSaving}
                    />
                    <button
                      type="button"
            ...
            >
              ...
              <div style={{ display: "flex", gap: "1rem" }}>
                ...
                {originalStatus !== 'published' && (
                  ...
                  >
                    <svg style={{ width: "18px", height: "18px" }} viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.59 14.37a6 6 0 01-5.84 7.38v-4.8m5.84-2.58a14.98 14.98 0 006.16-12.12A14.98 14.98 0 009.631 8.41m5.96 5.96a14.926 14.926 0 01-5.93 2.58m-.09-8.54a14.926 14.926 0 015.93-2.58M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>
                    Publish Now
                  </button>
                )}
