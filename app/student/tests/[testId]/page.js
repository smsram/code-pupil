...
export default function TestOverview() {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      if (test.studentStatus === 'in-progress') {
        ...
        const remainingSeconds = Math.floor(((endTime - now) % (1000 * 60)) / 1000)
        if (remainingMinutes >= 0) {
          setTimeUntilStart(`Time remaining: ${remainingMinutes}:${remainingSeconds.toString().padStart(2, '0')}`)
        } else {
      ...
      if (now >= startTime && now <= endTime) {
        ...
        setCanStart(true)
        const remainingMinutes = Math.floor((endTime - now) / (1000 * 60))
        setTimeUntilStart(`Test is LIVE - ${remainingMinutes} minutes remaining`)
        ...
        if (timeDiff <= 10 * 60 * 1000) {
          setTestState('ready')
          setCanStart(true)
          setTimeUntilStart(`Starts in ${minutes}:${seconds.toString().padStart(2, '0')}`)
        } else {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        if (studentTestsResponse.ok && studentTestsData.success) {
          ...
          const thisTest = studentTestsData.data.find(t => t.test_id === testId)
  ...
  if (isLoading || !test) {
    return (
      <>
        <LoadingOverlay active={isLoading} message="Loading test details..." type="spinner" blur={true} />
  ...
  return (
    <>
      <LoadingOverlay active={isLoading} message="Starting test..." type="spinner" blur={true} />
            ...
            <div className="student-logo-section">
              <div>
                ...
                <span className="student-logo-subtitle">
                  {testState === 'can-resume' ? 'Resume Test' : 'Test Instructions'}
                </span>
              </div>
            </div>
        ...
        <main className="student-overview-container">
          <div className="student-fade-in">
            ...
            {testState === 'can-resume' && (
              <div className="student-active-test-alert" style={{ marginBottom: 'var(--student-space-xl)' }}>
                <div className="student-alert-content">
                  ...
                  <div className="student-alert-info">
                    ...
                    <p className="student-alert-subtitle">
                      You have an active test session. Click "Continue Test" to resume where you left off.
                    </p>
                    <div style={{ marginTop: '0.5rem', color: 'var(--student-text-muted)' }}>
            ...
            {/* Locked Alert */}
            {testState === 'locked' && (
              <div className="student-active-test-alert" style={{ 
                ...
                <div className="student-alert-content">
                  ...
                  <div className="student-alert-info">
                    <h3 className="student-alert-title" style={{color: '#ef4444'}}>Test Locked</h3>
                    <p className="student-alert-subtitle">
                      This test has been locked by your instructor. Please contact them for more information.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="student-test-meta-grid">
                ...
                {test.score !== null && test.score !== undefined && (
                  <div className="student-meta-item">
                    ...
                    <div className="student-meta-value" style={{
                      color: test.score >= 80 ? '#22c55e' : test.score >= 60 ? '#f59e0b' : '#ef4444'
                    }}>
                      {test.score}%
                    </div>
                  </div>
            ...
            <div className="student-rules-section">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{ color: test.fullscreen_mode ? '#22c55e' : '#ef4444' }}>
                    {test.fullscreen_mode ? '✓' : '✗'}
                  </span>
                  <span>Fullscreen Mode</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{ color: test.auto_submit ? '#22c55e' : '#ef4444' }}>
                    {test.auto_submit ? '✓' : '✗'}
                  </span>
                  <span>Auto Submit</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{ color: test.show_results ? '#22c55e' : '#ef4444' }}>
                    {test.show_results ? '✓' : '✗'}
                  </span>
                  <span>Show Results</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{ color: test.allow_copy ? '#22c55e' : '#ef4444' }}>
                    {test.allow_copy ? '✓' : '✗'}
                  </span>
                  <span>Allow Copy</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{ color: test.allow_paste ? '#22c55e' : '#ef4444' }}>
                    {test.allow_paste ? '✓' : '✗'}
                  </span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span>📊</span>
                  <span>Similarity Threshold: {test.similarity_threshold}%</span>
                </div>
              </div>
            </div>
            ...
            <div style={{ textAlign: 'center', marginTop: '2rem' }}>
              {timeUntilStart && (
                <div style={{ 
                  ...
                  color: canStart ? 'var(--student-success-color)' : 'var(--student-warning-color)',
                  fontSize: '1.125rem',
                  fontWeight: 600
                }}>
                  {timeUntilStart}
              ...
              {testState === 'completed' ? (
                <button 
                  ...
                  style={{ pointerEvents: "auto", cursor: "pointer", background: '#22c55e' }}
                >
                  {getButtonText()}
                </button>
              ) : (
