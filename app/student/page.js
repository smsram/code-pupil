...
export default function StudentDashboard() {
  ...
  const fetchStudentData = async () => {
    ...
    try {
      ...
      // Fetch student tests - ADD TIMESTAMP TO PREVENT CACHING
  ...
  const createNotifications = (testsData) => {
    ...
    // Check for active tests (use backend status)
    ...
    if (activeTests.length > 0) {
      ...
      const remaining = Math.max(
        0,
        Math.floor((endTime - currentTime) / 60000)
      );
    ...
    // Check for live tests (use backend testStatus)
    ...
    // Check for upcoming tests (within 1 hour) - use backend data
    const upcomingTests = testsData.filter((t) => {
      if (t.testStatus !== "upcoming") return false;

      const timeUntil =
        new Date(t.start_time).getTime() - currentTime.getTime();
